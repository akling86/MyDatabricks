...
count = spark.sql("SELECT sum(total_orders) as total_orders FROM customer_counts WHERE country = 'Sweden'").collect()[0][0]
...
# MAGIC %timeit preZorderQuery = spark.sql("SELECT * FROM customer_data_delta where StockCode = 22301").collect()
...
# MAGIC %timeit postZorderQuery = spark.sql("SELECT * FROM customer_data_delta where StockCode = 22301").collect()
