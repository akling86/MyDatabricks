...
dfSource = (spark.read                        # The DataFrameReader
   .option("header", "true")       # Use first line of all files as header
   .option("sep", ":")            # Use tab delimiter (default is comma-separator)
   .option("inferSchema", "true")  # Automatically infer data types
   .csv(sourceFile)                   # Creates a DataFrame from CSV after reading in the file
   .withColumn('firstName2', lower(col('firstName')))
   .withColumn('middleName2', lower(col('middleName')))
   .withColumn('lastName2', lower(col('lastName')))
   .withColumn('ssn2', regexp_replace('ssn', '-', ''))
   .dropDuplicates(["firstName2","middleName2","lastName2","ssn2"])
   .drop("firstName2","middleName2","lastName2","ssn2")
)

# COMMAND ----------

display(dfSource)
display(dfSource.count())
